(() => {
  ...
  const UI = {
    ...
    sync() {
      // Синхронизация выполняется в фоне. Технические сообщения ученику не показываем.
    },
    loading() {
  ...
  const DataService = {
    ...
    async lessonIndex() {
      try {
        ...
        return Array.isArray(data.lessons) ? lessons : this.findSequential("lesson", "data/lessons", 200);
      } catch (error) {
      ...
      <section class="section" aria-label="Учебник">
        <div class="card book-card">
          ...
          <div>
            ...
            <h3>${Utils.escape([config.student.textbook, config.student.textbookEdition].filter(Boolean).join(" "))}</h3>
          </div>
        </div>
      </section>`;
  }
  ...
  async function initVocabulary() {
    ...
    const render = () => {
      ...
      if (window.__daniilVocabularyResizeHandler) window.removeEventListener("resize", window.__daniilVocabularyResizeHandler);
      window.__daniilVocabularyResizeHandler = scheduleVocabularyAlignment;
      window.addEventListener("resize", window.__daniilVocabularyResizeHandler, { passive: true });
    };
